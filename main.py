...
async def _select_schedule_post(page: Page):
    ...
    dropdown_opened = await page.evaluate("""() => {
        ...
        const candidates = [
            document.querySelector('.cm-dropdown__btn'),
            ...Array.from(document.querySelectorAll('button, [role="button"]'))
                .filter(el => el.innerText && el.innerText.trim().includes('立即發文') && isVisible(el)),
        ];
        for (const btn of candidates) {
            if (btn && isVisible(btn)) { btn.click(); return btn.className || btn.innerText.trim(); }
        }
        return null;
    }""")
    if dropdown_opened:
...
async def _wait_for_article_id_response(page: Page, timeout_s: int = 10) -> str:
    """攔截 CMoney 發文 API 回應，嘗試直接取得 article ID。"""
