...
async def _set_stock_tag(page: Page, tag_query: str = STOCK_TAG):
    ...
    if await search_input.count() > 0 and await search_input.is_visible():
        ...
        if not clicked:
            # 找不到加權指數結果 → 直接關閉 Modal，略過標記
            ts(f"  [略過] 搜尋結果中找不到「{tag_query}」，不標記個股")
            try:
                return_btn = page.locator('button.modalSearch__return').first
                if await return_btn.is_visible():
                    await return_btn.click()
    ...
    for attempt in range(20):  # 最多 10 秒 (每 500ms)
        handled = await page.evaluate("""() => {
            ...
            for (const ov of overlays) {
                ...
                if (publishBtn) {
                    // 取消所有已選的 AI 推薦標記，再發佈
                    ov.querySelectorAll('button.modalPredictStock__tag--selected').forEach(btn => {
                        try { btn.click(); } catch(e) {}
                    });
                    publishBtn.click();
                    return 'PredictStock:發佈文章(已清除所有AI標記)';
                }
